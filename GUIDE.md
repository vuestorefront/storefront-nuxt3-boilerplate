...
The boilerplate comes with a basic setup for cache control. The caching configuration can be extended in the `nuxt.config.ts` file. Use `routeRules` to define the caching rules for specific routes.

As the basic setup is not caching images generated by `NuxtImg`, the config can be extended to cache the images as well by setting proper headers.
