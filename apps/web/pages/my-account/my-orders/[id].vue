<template>
  <UiOverlay visible>
    <SfModal
      ...
      as="section"
      role="dialog"
      class="h-full w-full overflow-auto md:h-fit max-h-[100%] md:max-w-[770px] !px-0 min-w-650px"
    >
      <header class="fixed w-full pl-4 pr-2 top-0 py-2 z-10 bg-white items-center typography-headline-4 font-bold">
        <div class="flex justify-between">
          <h1 class="my-2">{{ $t('account.ordersAndReturns.orderDetails.heading') }}</h1>
          <SfButton square variant="tertiary" :tag="NuxtLink" :to="paths.accountMyOrders">
            <SfIconClose class="text-neutral-500" />
          </SfButton>
        </div>
      </header>
      <main class="mt-10 px-4 md:px-10">
        ...
        <table class="hidden md:table w-full text-left typography-text-sm mx-4 md:mx-0">
          ...
          <tbody>
            <tr v-for="(product, i) in products" :key="i" class="border-b border-neutral-200">
              <td class="pb-4 pr-4 lg:whitespace-nowrap"><UiProductCardVertical :product="product" /></td>
              <td class="p-4 lg:whitespace-nowrap">$295.87</td>
              <td class="p-4">1</td>
        ...
        <div class="flex justify-between border-t pt-4 border-neutral-200">
